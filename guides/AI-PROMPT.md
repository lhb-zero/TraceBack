...
3. LLM 会：读取项目 → 自主判断输出哪些文件 → 在当前目录直接创建文件 → 输出核对清单
4. 把生成的项目文件夹（`content/retros/{year}/{slug}/`）**整个复制**到 TraceBack 项目的 `content/retros/{year}/` 下（按 AI 输出的核对清单逐项核对）
5. **复核"我学到了什么"**：该节是 AI 基于项目证据起草的，务必用你的真实感受替换/补充——这是整个复盘对你唯一不可替代的价值
...
文件全部创建完后，在回复末尾输出一份清单，方便作者复制后逐项核对：
- **文件清单**：每个文件的路径 + 一句话说明
- **sub_docs 对应**：index.mdx 中 sub_docs 声明与实际文件的对应关系
- **踩坑去向**：通用坑文件清单（content/pitfalls/ 下）与项目内 pitfalls.mdx 的区分
- **待作者确认项**：标出"我学到了什么"里哪些是 AI 推断、哪些必须由作者补充
...
> **字段白名单**：frontmatter 只允许使用以上列出的字段，**禁止发明新字段**（如 commit_range、author 等）。确实有字段表达不了的信息，放进正文或标 [信息不足]。
...
6. "我学到了什么"部分：用第一人称、语气自然**起草**（基于项目证据推断的收获），但必须在 index.mdx 该节开头标注「此节为 AI 基于项目证据起草，请作者复核替换为真实感受」——AI 的推断不等于作者的内化，作者的真实感受是唯一不可替代的部分
7. commit hash、PR 链接必须是项目真实存在的，填进 frontmatter 对应字段
8. **事实锚定**：所有数字/性能数据/commit hash/时间线必须来自项目证据（git log / 代码 / 文档），无法溯源的一律标 [信息不足]，禁止用"大约/可能"糊弄
9. **字段白名单**：frontmatter 只允许使用"输出规范"中列出的字段，禁止发明新字段（见输出规范说明）
10. 生成完成后自检：必填字段齐全、枚举值合法、五个二级标题齐全、sub_docs 已声明、无超白名单字段、通用坑已进入 content/pitfalls/ 独立目录
